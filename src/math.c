...
void normalizevec4(vec4_t* v) {
  GLfloat sqr = v->m[0] * v->m[0] + v->m[1] * v->m[1] + v->m[2] * v->m[2];
  if(sqr == 1 || sqr == 0)
    return;
  GLfloat invrt = 1.f/sqrt(sqr);
  v->m[0] *= invrt;
  v->m[1] *= invrt;
  v->m[2] *= invrt;
}
...
mat4_t perspective(GLfloat fovy, GLfloat aspect_ratio, GLfloat near_plane, GLfloat far_plane) {
  ...
  const GLfloat y_scale = (GLfloat)(1/cos(fovy * PI / 360));
  const GLfloat x_scale = y_scale / aspect_ratio;
  const GLfloat frustum_length = far_plane - near_plane;
